...
export const productBlockCssString = `a.product-block {
  @apply flex w-80 flex-col self-stretch overflow-hidden rounded border-0 border-gray-300 bg-white shadow-xl transition-all dark:border-gray-700 dark:bg-gray-800;
}
...
a.product-block > div.caption {
  @apply relative flex flex-1 flex-col justify-between px-3 pb-6 pt-3 transition hover:bg-slate-400/5 dark:hover:bg-white/5;
}
...
a.product-block > div.caption > section > div.price-and-ratings > div.prices > span.price.sale {
  @apply text-lg font-semibold tracking-tight text-rose-600 hover:underline;
}
...
a.product-block > div.caption > section > div.price-and-ratings > div.ratings > span.sold-a-lot {
  @apply text-sm font-bold tracking-tight text-gray-500 hover:underline dark:text-gray-400;
}
...
a.product-block > div.caption > footer > div.colors > button.color {
  @apply h-6 w-6 rounded-full transition-all duration-200 ease-in-out hover:opacity-80;
}
